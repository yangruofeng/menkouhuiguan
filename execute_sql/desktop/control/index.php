...
class indexControl {
    ...
    protected $pwd_key = 'kiguue==**&773jksjhfiiegeie98377442--34989847773%$$@###!$@%^&@&AEHsoeannvbx';
    ...
    public function submit_scriptOp()
    {
        ...
        $author_password = getConf('author_password');
        if( $author_password == '' ){
            $data['state'] = false;
            $data['message'] = 'Please set password.';
            $data['remaining'] = '';
            echo json_encode($data);die;
        }

        if( md5($this->pwd_key.$password) != $author_password ){
            $data['state'] = false;
    ...
    public function updateOp() {
        ...
        if ($_POST) {
            ...
            $author_password = getConf('author_password');
            if( $author_password == '' ){
                showMessage('Please set password!');
            }
