...
class tableModelBase extends ormDataTable{
    ...
    public $ListOrderBy="";
    public $ListHaving="";
    public function __construct($_name,$_dsn=null,$reuseConnection=true){
        if(!$_name) $_name=get_class($this);
        if ($_dsn && $_dsn instanceof ormConnection) {
            $conn = $_dsn;
        } else {
            $conn = ormYo::Conn($_dsn,$reuseConnection);
        }
        parent::__construct($_name, $conn);
        $this->setChildren();
    }
