...
spl_autoload_register('classLoader::load_file');
...
function template($tplpath)
{
    if (strpos($tplpath, ':') !== false) {
        ...
        $pth = PROJECT_ROOT.'/common/' . $tpltmp[1] . '.php';
        return $pth;
    } else {
...
function getLiteUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, LITE_URL);
}
...
function getOperatorUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, OPERATOR_SITE_URL);
}
...
function getHelperUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style,HELPER_SITE_URL);
}
function getReportUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style,REPORT_SITE_URL);
}
function getDevUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, DEV_SITE_URL);
}
...
function getEntryUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, ENTRY_DESKTOP_SITE_URL);
}
function getHrUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, HR_SITE_URL);
}
function getScriptUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style,SCRIPT_SITE_URL);
}

function getTestUrl($cls, $method, $parameter = array(), $pseudo_static_style = false)
{
    return getUrl($cls, $method, $parameter, $pseudo_static_style, TEST_SITE_URL);
}
...
function dueDateFormat($dueDate) {
    if (preg_match('/^\d{1,2}$/', $dueDate)) {
        $d = intval($dueDate);
        if ($d == 1) {
            return "The {$dueDate}st of each month";
        } else if ($d == 2) {
            return "The {$dueDate}nd of each month";
        } else if ($d == 3) {
            return "The {$dueDate}rd of each month";
        } else {
            return "The {$dueDate}th of each month";
        }
    } else {
        return $dueDate;
    }
...
function generateGuid($uid = 0, $guid_type, $addition_id=null, $addition_pad=3)
{
    $uid = intval($uid);
    ...
    if ($addition_id === null) {
        ...
        $addition_seg = str_pad($addition_id, $addition_pad, '0', STR_PAD_LEFT);
    }
    $guid = $guid_type . $addition_seg . str_pad($uid, 6, '0', STR_PAD_LEFT);
    return intval($guid);
}
...
function getHrFileUrl($file_path)
{
    if (!$file_path) {
        return '';
    }
    return _UPLOAD_ . '/hr/' . $file_path;
}
 ...
 * 格式化浏览器输出调试信息
 * add by Maluer
 * 后面需要调整一下，让对象和数组可以进行打开关闭，暂时没想到好的处理办法,还有字体也有点刺眼，没找到清晰的字体
 * @param $data 需要输出的调试信息
 * @param bool $is_stop 输出之后是否停止程序执行
 */
function fm_print($data, $is_stop = true)
{
    ob_clean();
    ob_start();
    var_dump($data);
    ...
    $front_tag = "<div style='" . $style . "'>";
    $back_tag = '</div>';
    $output = $front_tag . '<pre>' . $output . '</pre>' . $back_tag;
    if ($is_stop) {
        echo $output;
        die;
    }else{
...
function commonExportToCSV($title, $headArr, $exportData)
{
...
/**  处理格式化输入的数据的round，如100,203.56这种格式的数据
 * @param $amount
 * @param int $precision
 * @return float
 */
function f_round($amount,$precision=2){
