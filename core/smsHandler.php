...
class smsHandler
{
     ...
     * @return result
     * */
    public function sendVerifyCode($phone, $code,$type=0)
    {
        ...
        //第一步插入数据库
        $insert = $this->insertSmsTask(smsTaskType::VERIFICATION_CODE, $phone, $content,$type);
        ...
        if (C("sms_api") == 'yunpian') {
            ...
            if (isset($st['code']) && $st['code'] == 0) {
                ...
                $task->update_time = Now();
                $task->update();
                return new result(false, 'Send sms fail:'.$st['msg'],$task);
            }
            ...
            if (isset($st['result']) && $st['result'] == 0) {
                ...
                $task->update_time = Now();
                $task->update();
                return new result(false, 'Send sms fail:'.$st['errmsg'],$task);
            }
    ...
    public function sendLoanRepaymentAlert($country_code,$phone_number,$payment_date,$payment_detail)
    {
        ...
        $insert = $this->insertSmsTask(smsTaskType::LOAN_REPAYMENT_ALERT, $contact_phone, $content,
            smsSendTypeEnum::MEMBER_LOAN_REPAYMENT_ALERT);
    ...
    private function insertSmsTask($type, $phone, $content,$category_type=null)
    {
        $m = M("common_sms");
        $row = $m->newRow();
        $arr_phone = tools::separatePhone($phone);
