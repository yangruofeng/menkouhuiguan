...
class ormYo{
    ...
    public static function Conn($dsn=null,$reuse=true){
        ...
        if ($reuse) {
            ...
            return new ormConnection($dsn_key,$dsn_value,ormYo::$driver_name);
        }
    }
    /*
     * @return dbMysqlI
...
class ormDataTable implements IormObj{
    ...
    public function select($item,$sql=null,$primary_key=true){
        $ret=$this->getRows($item,$sql);
        $new_ret=array();
        //自动转成数组
        if(count($ret)){
            foreach($ret as $k=>$v){
                if( $primary_key && $v[ormYo::$IDField]){
    ...
    private function _checkRow(ormDataRow $row){
        ...
        foreach($this->columns as $col){
            ...
            if($col->is_notnull && $row[$col->name]===NULL){
                return new ormResult(false,ormResult::ERROR_NOTNULL,$this->name.':'."Empty values are not allowed:".$col->name,$col->name);
            }
            /* 这里可能有些人对空的理解不一样。
            //字符串认为0长度的字符串也是空
            ...
            if(preg_match("/^INT|FLOAT|DOUBLE|DECIMAL|BIGINT$/i",$col->dataType)){
                ...
                if(!$row[$col->name] && $col->default===NULL && $isAdd){
                    ...
                    if($row[$col->name] && !is_numeric($row[$col->name])){
                        return new ormResult(false,ormResult::ERROR_INVALID_TYPE,$this->name.'->'.$col->name.':'."Data type does not match:".$col->dataType.",value:".$row[$col->name],$col->name);
                    }
                }
            }
            ...
            if(preg_match('@^(VARCHAR|CHAR)$@i',$col->dataType) && is_numeric($col->dataLen) && mb_strlen($row[$col->name],'utf8')>$col->dataLen){
                return new ormResult(false,ormResult::ERROR_MAXLEN,$this->name.":Out of length:".$col->dataLen." of ".$col->name,$col->name);
            }
            //检查日期字段
            if(preg_match("/^(DATE|DATETIME|TIMESTAMP)$/i",$col->dataType)){//日期字段
                if($row[$col->name]){
            ...
            if($col->dataType==ormDataColumn::DATATYPE_ENUM && $row[$col->name]){
                if(is_numeric($row[$col->name])){
                    if(!in_array($row[$col->name],$col->enum)){
                        return new ormResult(false,ormResult::ERROR_ENUM_LIMIT,$this->name.":Values are not within the enumeration range:".$col->name,$col->name);
                    }
                }else{
                ...
                if(is_array($kv)){
                    ...
                    switch(strtolower($kv[0])){
                        ...
                        case "time":
                            ...
                            break;
                        case "sql":
                            $v=(new ormParameter($this->name,'sql',$kv[1],'sql'))->toArray();
                            break;
                        default:
...
class ormSqlCommand{
    ...
    public function run(){
        if(!$this->commandText) return null;
...
class dbMysql implements IormDriver{
    ...
    private function open(){
        if(!$this->conn){
            ...
            if(!$this->conn){
                throw new Exception("Connect DataBase Failed:".	mysql_error());
            }else{
...
class dbMysqlI implements IormDriver{
    ...
    public $last_queryID;
    private $transaction=-1;
    private $transaction_hierarchy = 0;
    function __construct($_host,$_port,$_dbname,$_user_name,$_pwd){
        $this->host=$_host;
        $this->port=$_port;
        $this->dbname=$_dbname;
    ...
    private function open(){
        if(!$this->conn){
            ...
            if($this->conn->connect_error){
                throw new Exception("Connect DataBase Failed:".	$this->conn->connect_error, $this->conn->connect_errno);
            }else{
//                $r=$this->conn->select_db($this->dbname);DataBasextask
    ...
    public function startTransaction(){
        if($this->transaction_hierarchy===0){
            $this->open();
            $this->conn->autocommit(false);
            $this->transaction=0;
        }
        $this->transaction_hierarchy += 1;
    }
    public function submitTransaction(){
        $this->transaction_hierarchy -= 1;
        if ($this->transaction_hierarchy === 0) {
